...
Latest inspected GDS line: `2.6.4` from `@sovereignsquad/gds`.
...
These files are temporary product adapters while Habigoal migrates fully to `@sovereignsquad/gds`, `@sovereignsquad/gds-eslint-config`, and `@sovereignsquad/gds-compliance`. Feature surfaces should import GDS directly instead of importing generalized local wrappers from `components/ui`.
...
- `@sovereignsquad/gds`: unified runtime package for provider, theme, public/core primitives, admin/workspace primitives, semantic actions, state blocks, forms, responsive data views, upload/media, and recovery flows.
- `@sovereignsquad/gds-eslint-config`: shared lint enforcement for GDS drift.
- `@sovereignsquad/gds-compliance`: manifest and repo-level compliance validation.
...
- The GDS packages are live on npm under `@sovereignsquad/*` at `2.6.4`.
...
Use `@sovereignsquad/gds/server` for server-safe App Router composition and `@sovereignsquad/gds/client` for providers, hooks, and interactive surfaces.
...
2. **Registry adoption:** Keep the approved npm registry source for `@sovereignsquad/gds`, `@sovereignsquad/gds-eslint-config`, and `@sovereignsquad/gds-compliance`.
...
5. **Protected workspace migration:** Keep the protected workspace on GDS `AppShell` and continue migrating stats strips, settings tables, restore views, and CRUD layouts to `@sovereignsquad/gds-admin` component families. Core action controls use GDS `SemanticButton` where the action vocabulary is exact.
...
7. **Enforcement:** Add `@sovereignsquad/gds-eslint-config`, `@sovereignsquad/gds-compliance`, adoption-manifest validation, and an expanded `npm run semantic:audit` gate that fails new local token authority, raw colors, and non-GDS generalized UI primitives.
