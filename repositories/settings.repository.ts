...
export async function getGlobalSettings(): Promise<KidexSettings | null> {
  const db = await getDatabase();
  const settings = await db.collection(collectionName).findOne({ _id: SETTINGS_ID as any });
  if (!settings) return null;
  const { _id, ...rest } = settings;
  return rest as unknown as KidexSettings;
}

export async function updateGlobalSettings(settings: KidexSettings) {
  const db = await getDatabase();
