...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.info(JSON.stringify({
      event: "aiq.trainer.athletes.loaded",
      athleteCount: projection.athletes.length,
      correlationId,
      latencyMs: Date.now() - startedAt
    }));
    return NextResponse.json({
      athletes: projection.athletes,
      dataFreshness: {
        generatedAt: new Date().toISOString(),
        sourceCollections: ["children", "teams", "athleteiq_daily_iq_snapshots", "habit_records", "coach_actions"]
      },
      correlationId,
      latencyMs: Date.now() - startedAt
    });
