...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.info(JSON.stringify({ capabilityKey: ATHLETEIQ_READINESS_ROUTE_CAPABILITY_KEY, event: "athleteiq.readiness_route.recalculated", correlationId, athleteIdHash: athleteIqHashForLog(input.athleteId), route: snapshot.route, latencyMs: Date.now() - startedAt }));
