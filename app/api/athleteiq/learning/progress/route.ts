...
import { createLearningProgress } from "@/services/athleteiq-lite-modules.service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await createLearningProgress({ body, actorEmail: user.email, idempotencyKey: getIdempotencyKey(request, body) });
    if (result.errors.length) return athleteIqJsonError("VALIDATION_ERROR", 400, correlationId, { details: result.errors });
    console.info(JSON.stringify({ capabilityKey: ATHLETEIQ_LITE_GATEWAY_CAPABILITY_KEY, event: "athleteiq.learning_lite.progress_saved", correlationId, athleteIdHash: athleteIqHashForLog(body.athleteId), latencyMs: Date.now() - startedAt }));
