...
import { createManualWearableEntry } from "@/services/athleteiq-lite-modules.service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await createManualWearableEntry({ body, actorEmail: user.email, idempotencyKey: getIdempotencyKey(request, body) });
    if (result.errors.length) return athleteIqJsonError("VALIDATION_ERROR", 400, correlationId, { details: result.errors });
    console.info(JSON.stringify({ capabilityKey: ATHLETEIQ_LITE_GATEWAY_CAPABILITY_KEY, event: "athleteiq.wearable_manual.entry_saved", correlationId, athleteIdHash: athleteIqHashForLog(body.athleteId), warnings: result.warnings, latencyMs: Date.now() - startedAt }));
