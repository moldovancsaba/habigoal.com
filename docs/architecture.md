...
**Weekly versioned snapshots (#86).** `lib/weekly-snapshot.ts` (`buildWeeklySnapshot`) composes a report (with its provenance), habit scoring, load summary, reflection themes, and insight signals (#81) into one reproducible `WeeklyAthleteSnapshot` that captures the **scorer / report / insight / snapshot versions** and **source counts** that produced it. `snapshotVersionsMatch` flags a stored snapshot as stale when any version differs from the current code, so regeneration is explicit rather than silent drift. Snapshots persist in `weekly_snapshots` (one per `athleteId`+`weekStart`) via `services/weekly-snapshot.service.ts`; `GET/POST /api/athletes/[id]/weekly-snapshots` list/generate, and `GET …?weekStart=&format=json` performs an authorized, audited export. All routes are athlete-scoped (`report:read` to read, `report:export` to generate/export).
...
**Single sign-on across the three surfaces (selector, Habigoal, Athlete IQ).** One shared session cookie (`habigoal_session`, path `/`, 30-day sliding refresh) authenticates all surfaces. The login page passes an existing valid session straight through to its `next` target when the user can open the requested surface, so a signed-in user never re-authenticates when switching apps or personas — login is permanent until explicit logout. The pass-through is skipped when an `error` param is present so an entitlement bounce from `requireProductSession` shows its message instead of looping. The landing selector offers three pre-selectable destinations — Habigoal, Athlete IQ (athlete), Athlete IQ (trainer) — each encoding persona + surface so first-time login arrives pre-selected and a returning user lands directly in the app. The two Athlete IQ entries carry the persona into the destination (`/athlete-iq?persona=…`); `getAthleteIqProductDashboardProjection` honours it only when the user actually holds the role (an athlete can never view the trainer surface, nor vice-versa), so the two cards render visibly different experiences — the athlete workspace vs the trainer/team command surface — falling back to the user's primary role when no persona is requested.

Key surfaces:

- `/dashboard` — coach triage and recommendation surface.
- `/dashboard/planning` — coach weekly planning from live readiness/load state; weekly plans persist in `session_plans`. Reusable **session blueprints** (TRN-002, #83) — ordered, timed drill sets per variant (standard/controlled/recovery) — live in `lib/session-blueprints.ts` and are served read-only at `GET /api/session-blueprints`. A pure, client-only timer reducer (`lib/session-timer.ts`, play/pause/skip/tick/reset) drives execution; only the final debrief is persisted. The athlete runner lives at `/athletes/[id]/session` (`components/product/session/SessionRunner.tsx`): it loads blueprints, runs the timed drills with an `aria-live` timer announcement and labelled controls, then creates a draft session via `POST /api/athleteiq/sessions` (`createSessionFromBlueprint` → `buildSessionFromBlueprint`, mapping drills to blocks within the existing session lifecycle — no parallel module) and saves the debrief (RPE/completion/pain/mood/notes) through the existing `…/sessions/[id]/debrief` route, which links it to training load.
