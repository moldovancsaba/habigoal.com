...
- Latest inspected GDS version: `3.9.0`
- Latest inspected GDS package: `@sovereignsquad/gds`

## What GDS Provides

The General Design System repo now exposes the unified product package plus governance packages:

- `@sovereignsquad/gds`: unified runtime package exposing theme, core, admin, server, and client entrypoints.
- `@sovereignsquad/gds-eslint-config`: shared lint enforcement for raw design values and forbidden drift.
- `@sovereignsquad/gds-compliance`: shared manifest and repository-level compliance validation.
...
Habigoal has the right foundation choice, Mantine, and now passes the shared `@sovereignsquad/gds-compliance` repository check. The protected dashboard shell now consumes the GDS `AppShell` contract through a thin Habigoal adapter that supplies role-aware navigation, account controls, locale switching, and theme persistence.
...
- GDS packages are published on npm under the real `@sovereignsquad/*` namespace.
- Local GDS package version inspected: `3.9.0`.
- GDS owns the Mantine/Tabler engine install surface through `@sovereignsquad/gds`; Habigoal does not declare those engine packages directly.
- Package overrides pin the auto-installed Mantine peers to the GDS-supported `8.3.x` lane until temporary direct Mantine imports are removed from server-rendered App Router files. Mantine `9.x` imports React client-only exports that are unavailable through the `react-server` condition.
- Habigoal now installs the unified `@sovereignsquad/gds` package from npm.
- The root provider is now `GdsProvider` from `@sovereignsquad/gds/client`, wrapped by the Habigoal theme-mode adapter.
...
- App pages now use `PageHeader`, `SectionPanel`, `StateBlock`, and `FormField` from `@sovereignsquad/gds/client` directly where the GDS contract is compatible.
...
Use the live npm packages directly: `@sovereignsquad/gds`, `@sovereignsquad/gds-eslint-config`, and `@sovereignsquad/gds-compliance` at `3.9.0`. Do not reintroduce sibling `file:` package links, split runtime dependencies, the former package scope, or the old placeholder package namespace.
...
```ts
import { gdsTheme, extendGdsTheme, PageHeader, PublicShell, ArticleShell, WorkspaceHeader } from "@sovereignsquad/gds/server";
```

```tsx
"use client";

import { GdsProvider, SemanticButton, ThemeToggle, AppShell, ResponsiveDataView } from "@sovereignsquad/gds/client";
```
...
- Done: consume `@sovereignsquad/*` packages from a stable registry source.
- Keep Mantine on one supported major.
- Add a lockfile check that fails duplicate Mantine majors and mixed GDS versions.
- Keep `@sovereignsquad/gds-eslint-config` and `@sovereignsquad/gds-compliance` wired, then expand them from migration gates into release gates.

### Phase 2: Root Provider

- Done: replace local Mantine provider ownership with `GdsProvider` from `@sovereignsquad/gds/client`.
...
The `@sovereignsquad/gds@3.9.0` package now covers these Habigoal non-standard elements:
...
- Replace remaining admin tables, settings sections, restore bin, governance cards, and CRUD surfaces with admin/workspace contracts from `@sovereignsquad/gds`.
...
- GDS packages are installed from npm under the unified `@sovereignsquad/gds` package line.
- The root provider uses `GdsProvider` from `@sovereignsquad/gds/client`.
