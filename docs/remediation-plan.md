...
- ✅ #283 delivered (measurement adapter layer): `lib/measurement-normalization.ts` adds the cross-cutting metadata the canonical metric model lacked — `sourceTrust(source)` (device/manual/inferred), `classifyFreshness`/`ageHours` (fresh/stale/expired vs cadence thresholds), and `describeMeasurement(metric, now)` which returns a render-ready descriptor with an honest `cautioned` flag (set when stale, low/unknown confidence, or AI-inferred) so lite/future/derived values are never shown as active verified measurements. Covered by `lib/measurement-normalization.test.ts`.
...
- ✅ #209 delivered: `lib/engines/recovery.engine.ts` now merges **subjective** (stress, soreness) and **wearable** (sleep quality, HRV) recovery signals into the composite score. Each signal contributes only when present; absent ones are tracked in `missingData` and lower confidence, and a wearable signal (sleep or HRV) is required for `high` confidence — no signal is fabricated when missing. Covered by `lib/engines/recovery.engine.test.ts`.
...
- ✅ #334 delivered: shipped surfaces are already free of `lorem`/`ipsum`/dummy content (prior mock-removal work). Added the CI guard `tests/no-placeholder-content.test.ts` (scans `app/`, `components/`, `messages/`, fails on filler markers) to hold the line, plus `docs/placeholder-content-policy.md` (banned list + allowed form-guidance placeholder catalog; documents why `demo`/`sample` aren't banned wholesale — the #427 demo seed creates real records, not placeholders).
