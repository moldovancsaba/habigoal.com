...
| `components/theme/ThemeModeContext.tsx` | `mode` / `setMode`, syncs `document.documentElement` `data-theme` and `localStorage`. |
| `components/layout/DashboardShell.tsx` | Responsive shell: `AppBar` + temporary `Drawer` (mobile) + permanent `Drawer` (desktop), nav, `PageContainer`. |
...
- Record print view relies on `className="no-print"` / `only-print` and global `@media print` rules that hide the MUI drawer/app bar and expand `.dashboard-main`.
